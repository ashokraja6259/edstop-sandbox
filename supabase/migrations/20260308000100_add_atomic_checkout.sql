-- Add idempotency key for safe checkout retries
...
-- Creates order + order items + wallet deduction + wallet ledger + order event in one transaction.
...
BEGIN
  ...
  -- Promo validation + discount calculation inside transaction
