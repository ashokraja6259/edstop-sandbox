-- Deliver the JSONB checkout normalization to databases that already recorded
-- 20260308000108. Editing a historical migration repairs clean installs but
-- does not update an existing database, so this additive migration is the
-- authoritative production replacement for create_order_atomic.
...
BEGIN
  ...
  IF v_payment_method IN ('razorpay', 'online') THEN
    RAISE EXCEPTION 'Online payment verification is not enabled for food orders yet';
  END IF;

  IF v_payment_method <> 'cod' THEN
    RAISE EXCEPTION 'Unsupported food order payment method. Use cod with wallet redemption when needed.';
  END IF;
  ...
  IF p_promo_code IS NOT NULL AND LENGTH(TRIM(p_promo_code)) > 0 THEN
    ...
    ELSE
      v_promo_discount := ROUND(
        (v_subtotal * COALESCE(v_promo.discount_value, 0) / 100)::NUMERIC,
        2
      );
  ...
  IF v_wallet_requested > 0 THEN
    ...
    v_wallet_used := LEAST(
      v_wallet_requested,
      GREATEST(0, v_subtotal - v_promo_discount)
    );
  ...
  v_final_amount := GREATEST(
    0,
    v_subtotal - v_promo_discount - v_wallet_used
  );
  v_order_number :=
    'ED'
